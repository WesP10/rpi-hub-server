...
class FlashTask(BaseTask):
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        # Detect if this is .ino source code (vs compiled .hex)
        is_ino_source = self._is_ino_source(firmware_text)
        
        if is_ino_source:
            # Source code path: compile then flash
            firmware_path = await self._compile_and_prepare_firmware(
                firmware_bytes=firmware_bytes,
                board_fqbn=self.board_fqbn or board_fqbn
            )
        else:
            # Hex file path: write to temp file
